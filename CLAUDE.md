...
Vite + React 做的簡報，給主管月會報告用。二維導覽：左右鍵換章節，上下鍵在章節內換步驟，共 7 章。
...
- **驗證強度要跟改動大小相稱**，不要每次都跑整套（使用者反映過太慢）：
  - **純文字增刪**（改文案、加減一條 bullet）：`npm run shot -- --only <章.步驟> --size 1440x700`，
    **不用打開截圖**。腳本會印出每頁高度、有沒有被切到，有問題自己 exit 1；
    數字正常就直接部署。
  - **改版面**（CSS、元件結構、換影片／圖）：三種尺寸都跑，**而且要真的看圖**。
    這種改動才會爆版、才會左右對不齊。
  - 使用者一次給多個修改：全部改完再驗證、再部署**一次**，不要每項各跑一輪。
- 改哪一章就 `--only <章>` 或 `--only <章.步驟>`，不用每次掃全部。
- 使用者對版面很敏感，會逐頁檢查。
- 溝通用繁體中文。
