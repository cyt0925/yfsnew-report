...
- [ ] **第 6 章（額外功能）還沒有影片**。簽名的第一頁右欄放的是 SOP 檢索網站上
      那份舊流程的截圖（`oms-sign-sop.png`），第二頁是純文字兩欄，採購表那頁
      維持滿版堆疊（4 個步驟 + 3 條「做到了什麼」，分欄後的 1.6rem 會爆版）。
      簽名那兩頁不是用 `OmsFeature`——那個模板的右欄固定是影片卡，這兩頁一個是
      靜態截圖、一個是純文字，另外寫成 `OmsSign` 反而單純，版面 class 是共用的。
