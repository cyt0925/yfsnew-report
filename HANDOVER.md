...
- **第 5 章五個功能頁一律走左右分欄**（`OmsFeature` 的 `layout` 預設值，不再是
  ...
  **影片卡的上緣也從那條線起跑，不是從大標題**。
  - 右欄 `align-self: start`，**高度由影片本身決定，不拉滿左欄**。
    試過拉滿（`stretch` + 字幕條 `flex:1`）：長的頁面字幕條被撐開、一句話的敘述
    浮在一大塊空白中間；短的頁面反過來把字幕條擠到剩幾個 px。現在字幕條是自然
    高度，文字跟卡片上下緣的距離就只由它的 `padding` 決定。
  - 影片維持原比例（`aspect-ratio: 1600/900` + `object-fit: cover`），不跟著卡片變形。
