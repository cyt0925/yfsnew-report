...
- **第 5 章五個功能頁一律走左右分欄**（`OmsFeature` 的 `layout` 預設值，不再是
  「有影片才切換」）。版面是 **grid 的兩列**：第一列只有大標題、壓在左半邊；
  第二列才分左右。所以「怎麼操作」跟「為什麼要做」對齊在同一條線上，
  **影片卡的上緣也從那條線起跑，不是從大標題**。
  - **左右兩欄的上下緣都要切齊**，這是使用者明確要求的。列高由左欄的文字決定，
    右欄 `align-self: stretch` 跟著拉滿：字幕條是自然高度（`flex: none`，文字跟
    卡片上下緣的距離只由它的 `padding` 決定），**影片吃掉剩下的全部高度**。
  - 所以影片**不鎖 16:9**。鎖了下緣就切不齊——實測兩個方向的落差都有，1920 下
    右欄長 142px、1366 下左欄長 90px。改用 `object-fit: cover` 填滿不變形，多的
    從上下（或左右）裁掉；這幾支螢幕錄影上緣本來就是分頁列與書籤列，裁掉更好讀。
  - **影片一定要絕對定位**（`.oms-video-frame` 包一層，影片 `position:absolute; inset:0`）。
    `<video>` 有自己的內在尺寸，留在流排版裡的話那個高度會被算進 grid 的列高，
    列高被右欄撐大，卡片就縮不到左欄的高度、下緣永遠差幾十 px。
    這一點卡了一輪才找到：光是 `flex:1 + min-height:0` 沒有用。
...
- 每次改完都要**實際跑起來用 Playwright 截圖確認**（排版有沒有溢出、字級、動畫），
  不要只看程式碼就說做好了。用 `npm run shot`（`scripts/shot.mjs`），改哪一章就
  `--only <章>`。有頁面被切到它會 exit 1。
  **不要每次重寫一次性的截圖腳本**——量測有兩個很容易踩的陷阱（24 頁同時掛在
  DOM 裡、縮放貼在 section 而不是 `.step` 上），腳本開頭有寫，重寫大概率再踩一次。
  我就踩過，還因此誤報過一次「全貌」章被切掉。
