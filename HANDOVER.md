...
| 6 | 額外功能 | 3 | `sections/oms/OmsSign` × 2 + `OmsPurchase` |
...
（原本各自一章，後來併起來）。三個步驟：簽名拆成兩頁（為什麼要做 + SOP 截圖 /
怎麼操作 + 做到了什麼），加上採購表格式轉換一頁。
...
- [ ] **第 6 章只剩採購表那頁沒有影片**。它維持滿版堆疊（4 個步驟 + 3 條
      「做到了什麼」，分欄後的 1.6rem 會爆版），而且沒有影片可以放右欄。
      簽名那兩頁的版面：
      - 第一頁**上下排**——為什麼要做在上，SOP 檢索網站上那份舊流程的截圖
        （`oms-sign-sop.png`）在下、橫跨整個內容欄。圖上的字很小，走滿版才讀得到。
      - 第二頁**左右分欄**——怎麼操作、做到了什麼兩塊都收在左欄，右欄放示範影片
        （`oms-sign-demo.webm`）。這頁的下緣對齊方式跟功能頁**相反**：功能頁是讓
        影片填滿剩下的高度（裁切），這頁的影片沒有字幕條可以吸收高度，硬填會裁掉
        將近三成、正中央那個簽名對話框就毀了，所以改成影片鎖 16:9 不動、左欄拉滿
        整列高度用 `space-between` 把兩塊文字撐開。代價是兩塊之間的空隙很大。
      簽名那兩頁不是用 `OmsFeature`——那個模板的左欄固定是「為什麼要做／做到了
      什麼」、影片下面固定掛字幕條，這兩頁的組合都不一樣，另外寫成 `OmsSign`
      反而單純，版面 class 是共用的。
