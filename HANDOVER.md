...
第 4 章第三頁「缺乏追蹤與防呆機制」是左右兩欄：左欄 8 格由上到下是三句回饋 →
結論 → 營運人員自己記的 Excel 總表（`roadmap-excel-sheet.png`，原比例不裁——右側
那串「8/21-Y新、8/24-Y新…」篩選選單就是「改了又改」的證據，走滿版會被裁掉）；
右欄 4 格是營運同仁的 LINE 對話（`roadmap-line-chat.png`），跟三句回饋齊頭。
兩張圖要用 `.compare-panel.feedback-*` 複合選擇器覆寫 `.compare-panel img` 鎖的
15.5rem 高，同權重會輸給寫在後面的預設值（踩過一次，圖兩側跑出黑邊）。

第 6 章「額外功能」是「驗收單自動簽名」跟「採購表格式轉換」合成的一章
（原本各自一章，後來併起來）。四個步驟，兩個功能各拆兩頁、結構一樣：
第一頁上下排（為什麼要做 + 一張滿版圖），第二頁左右分欄（怎麼操作在左、影片在右）。
簽名的圖是 SOP 檢索網站上的舊流程截圖；採購表的圖是 Before／After 對照
（瑪氏訂貨通知單 → 公司採購範本），第二頁左欄再放一次縮小版。

它刻意**不放進第 5 章的下滑動線**：這些是獨立產品，不是同一條流程裡的步驟。
軌道編號用 `03`（第 5 章是 `02`），眉標是「額外功能 01／02」。
...
- **第 5 章五個功能頁一律走左右分欄**（`OmsFeature` 的 `layout` 預設值，不再是
  ...
  **影片卡的上緣也從那條線起跑，不是從大標題**。
  ...
  - 資料裡多給一個 `still: { src, alt }`，會在影片跟字幕條中間夾一張橫幅截圖
    （功能 03 的批次操作列）：影片拍不到、但值得單獨看一眼的畫面就放這裡。
    它是固定高度（由卡片寬度決定），對列高的貢獻固定，不會破壞左右切齊。
...
- [x] ~~第 6 章的影片~~ 簽名（`oms-sign-demo.webm`）跟採購表（`oms-purchase-demo.webm`）
      都補上了，整份簡報的示範影片到齊。
      第 6 章兩個功能的版面（`OmsSign` / `OmsPurchase`，各自一個元件、`part` 0/1 兩頁）：
      - 第一頁**上下排**——為什麼要做在上、圖在下走滿版。簽名放 SOP 舊流程截圖
        （`oms-sign-sop.png`）；採購表放 Before／After 對照（`oms-purchase-before/after.png`），
        寬度上限 960px——Before 那張 1645×608 走滿版整頁會超過 963px。
      - 第二頁**左右分欄**——怎麼操作在左、影片在右。下緣對齊的做法兩個功能**不一樣**：
        - 簽名：左欄比影片矮 → 影片鎖 16:9 不裁，左欄 `space-between` 把兩塊撐開。
        - 採購表：左欄（怎麼操作 + 縮小版對照圖）比影片高 → 走功能頁那套，影片填滿
          卡片、`object-fit: cover`，裁的是**左右**（影片兩側是空的綠底，裁掉沒差）。
          來源影片本身上下有黑邊，這頁卡片底色改純黑（`.oms-purchase-detail .oms-video-card`）
          讓黑邊融進去。
      這兩個元件都沒有用 `OmsFeature`——那個模板的左欄固定是「為什麼要做／做到了
      什麼」、影片下面固定掛字幕條，這幾頁的組合都不一樣。版面 class 是共用的。
