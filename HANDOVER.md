...
- **整份簡報會跟著視窗高度等比縮小。** `html { font-size: clamp(9px, 1.684vh, 16px) }`——
  版面幾乎全是 rem 疊出來的，所以縮 rem 就等於縮整張投影片，比例與留白不變。
  基準是 950px 高（1.684vh = 16px ÷ 950px），高度 ≥ 950px 的桌機／投影機維持 16px，
  畫面跟以前完全一樣；筆電或系統顯示縮放 125%／150% 時才會縮。
  **新增頁面時的紅線：內容在 950px 高的視窗裡要塞得下**（目前最高的一頁是 SOP 比對頁 894px），
  塞得下就等於所有尺寸都塞得下。另外，高度不是 rem 撐出來的東西（寫死 px、
  或照原生比例縮放、高度只跟欄寬有關的圖）不會跟著縮，要另外用 rem 封頂——
  `.doc-pile` 的高度、`.maintain-gallery` 的 `max-width` 就是為了這件事。
